...
TEST_CASE("Black-Scholes-Merton model")
{
    ...
    SECTION("Imlpied volatility on European call option: Iterative using template functions")
    {
        auto func = BlackScholesMerton::valueCallOption;
        ...
        // Keep track of performance to analyze vs. other methods of finding implied vol
        auto start = std::chrono::system_clock::now();
        double impliedVol = BlackScholesMerton::valueImpliedVolatilityIterativeTemplate(func, stockPrice, strikePrice, risklessRate, duration, optionValue);
        auto end = std::chrono::system_clock::now();
        ...
        auto elapsed = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start);
        // convert manually because if we casted in milliseconds we would get 0 by rounding
        std::cout << "Iterative implied vol with templates took " <<  elapsed.count() / Utilities::NANO_TO_MILLI << " milliseconds.\n";
    }
    ...
    SECTION("Implied volatility on European call option: Bisection method using template funtions")
    {
        auto func = BlackScholesMerton::valueCallOption;
        ...
        // Keep track of performance to analyze vs. other methods of finding implied vol
        auto start = std::chrono::system_clock::now();
        double impliedVol = BlackScholesMerton::valueImpliedVolatilityBisectionTemplate(func, stockPrice, strikePrice, risklessRate, duration, optionValue);
        auto end = std::chrono::system_clock::now();
        ...
        auto elapsed = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start);
        // convert manually because if we casted in milliseconds we would get 0 by rounding
        std::cout << "Bisection method implied vol with templates took " <<  elapsed.count() / Utilities::NANO_TO_MILLI << " milliseconds.\n";
    }
    
    SECTION("Implied volatility on European call option: Bisection method using template functions")
    {
        auto func = BlackScholesMerton::valueCallOption;
    ...
    SECTION("Implied volatility on European call option: Newton-Raphson method using template functions")
    {
        auto func = BlackScholesMerton::valueCallOption;
        ...
        // Keep track of performance to analyze vs. other methods of finding implied vol
        auto start = std::chrono::system_clock::now();
        double impliedVol = BlackScholesMerton::valueImpliedVolatilityNewtonRaphsonTemplate(func, stockPrice, strikePrice, risklessRate, duration, optionValue);
        auto end = std::chrono::system_clock::now();
        ...
        auto elapsed = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start);
        // convert manually because if we casted in milliseconds we would get 0 by rounding
        std::cout << "Newton-Raphson method implied vol with templates took " <<  elapsed.count() / Utilities::NANO_TO_MILLI << " milliseconds.\n";
    }
